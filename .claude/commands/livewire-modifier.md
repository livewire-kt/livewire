...
### Modifier definition: `ui/src/commonMain/kotlin/com/r0adkll/livewire/ui/modifier/<Name>Modifier.kt`

This file contains the internal modifier class and the public extension function(s).

**Modifier class pattern:**
- Annotated with `@LivewireSerializer` (from `com.r0adkll.livewire.annotations.LivewireSerializer`) — this auto-registers the modifier with the KSP-generated polymorphic serializer, no manual registration needed
...
- All properties must be serializable. For non-serializable types:
  - `Color` → use `@Serializable(with = ColorSerializer::class)` annotation (import from `com.r0adkll.livewire.ui.graphics.ColorSerializer`)
  - `Dp` → store as `Float` (the raw `.value`), convert back with `.dp` in `toComposeUi`
  - `Shape` → use `com.r0adkll.livewire.ui.graphics.Shape` (already `@Serializable`)
...
- Package: `com.r0adkll.livewire.ui.modifier` for all modifier files
...
- Shape values: use `com.r0adkll.livewire.ui.graphics.Shape` (Livewire's serializable shape types: `RectangleShape`, `CircleShape`, `RoundedCornerShape`)
...
These types from `com.r0adkll.livewire.ui.graphics` are already `@Serializable` and can be used as modifier properties:
