...
class ParameterExtractorTest {
  @Test
  fun `extracts the declared parameter values from a restart lambda`() {
    val params = extract(
  ...
  @Test
  fun `reflects updated parameter values after multiple compositions`() {
  ...
  @Test
  fun `extracts the value of a defaulted parameter`() {
    val params = extract(
  ...
  @Test
  fun `skips the extension receiver field when extracting parameters`() {
...
private fun extract(scope: RecomposeScope, vararg metadata: ParameterSourceInformation) = extractParametersFromLambda(scope, metadata.toList())
