...
@Immutable
@Serializable
data class IntValueChangeAction(
  val identifier: String,
  val value: Int = 0,
) : LivewireAction

@Composable
fun intValueChangeAction(
  onValueChange: (Int) -> Unit,
): IntValueChangeAction {
  val compositionKey = "onIntValueChange_${currentCompositeKeyHashCode}"
  val updatedOnValueChange by rememberUpdatedState(onValueChange)
  val actionObserver = LocalLivewireActionObserver.current

  LaunchedEffect(compositionKey) {
    actionObserver.events
      .filterIsInstance<IntValueChangeAction>()
  ...
  return remember(compositionKey) {
    IntValueChangeAction(compositionKey)
  }
}
